...
def main(llm_type):
    ...
    def index():
        ...
        if request.method == "POST":
            ...
            answer_no_rag = rag_answer_question(question, results, llm_interface, use_rag=False)
        ...
        {% if display_results %}
            ...
            <h3>Generated Answer (No Rag):</h3>
            <pre style="white-space: pre-wrap;">{{ answer_no_rag }}</pre>
            <h3>Retrieved Shot Logs:</h3>
            <pre style="white-space: pre-wrap;">{{ retrieved_results }}</pre>
        {% endif %}
        """, question=question, retrieved_results=retrieved_results, answer=generated_answer, answer_no_rag=answer_no_rag, display_results=display_results)
